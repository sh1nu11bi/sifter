...
install(){
    if [[ -d '/root/.wine/drive_c/fuzzbunch' ]]; then
        ...
        sudo apt-get install xterm wine wine32 wine-development wine-binfmt winexe winetricks 
        ...
        sudo wine regedit
        sleep 5
        echo -e "${ORNG}When Fuzzbunch has finished downloading and xTerm \nwindow has closed, please hit ${LBBLUE}enter${NC}"
        read FUZZBUNCH
